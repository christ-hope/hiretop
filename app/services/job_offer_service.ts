...
export class JobOfferService {
  /**
   * Recuperation des offres d'emploi/jobs
   */
  ...
  async createJobOffer(userId: number, data: CreateJobData): Promise<JobOffer> {
    const companyMember = await CompanyMember.query().where('user_id', userId).firstOrFail()
  ...
  async updateJobOffer(jobId: number, data: UpdateJobData, userId: number): Promise<JobOffer> {
    ...
    if (data.contractType !== undefined) job.contract_type = data.contractType
